...
Section AxiomaticRiscvL.
  ...
  Class AxiomaticRiscvL :=  mkAxiomaticRiscvL {
      ...
      Bind_storeByte: forall {A: Type} (addr: t) (v: word 8)
          ...
          (f tt) (with_machineMem_log (Memory.storeByte initialL.(machineMem) addr v)
                                            initialL);

      Bind_storeHalf: forall {A: Type} (addr: t) (v: word 16)
                        (f: unit -> OState RiscvMachine A) (initialL: RiscvMachine),
          (Bind (storeHalf addr v) f) initialL =
          (f tt) (with_machineMem_log (Memory.storeHalf initialL.(machineMem) addr v)
                                            initialL);

      Bind_storeWord: forall {A: Type} (addr: t) (v: word 32)
                        (f: unit -> OState RiscvMachine A) (initialL: RiscvMachine),
          (Bind (storeWord addr v) f) initialL =
          (f tt) (with_machineMem_log (Memory.storeWord initialL.(machineMem) addr v)
                                            initialL);

      Bind_storeDouble: forall {A: Type} (addr: t) (v: word 64)
                        (f: unit -> OState RiscvMachine A) (initialL: RiscvMachine),
          (Bind (storeDouble addr v) f) initialL =
          (f tt) (with_machineMem_log (Memory.storeDouble initialL.(machineMem) addr v)
                                            initialL);
